...
export default function Index(props) {
    ...
    const submit = (e) => {
        ...
        post(route('activities-types.store'), { onSuccess: () => {
            reset();
            setVisible(false)
        } });
    };
        ...
        >
            ...
            <CRow>
                <div className="d-flex w-100 mb-3">
                    ...
                    <CModal backdrop="static" size="xl" visible={visible} onClose={() => setVisible(false)}>
                        <CModalHeader>
                            <CModalTitle>Adicionar Atividade</CModalTitle>
                        </CModalHeader>
                        <CModalBody>
                        <CForm
                ...
                <CCol lg={7}>
                    { activity_types && activity_types.map( ({ id, title, options }) => (
                        <CCard key={id} className="mb-3">
                            <CCardHeader>
                                {title} #{id}
                            </CCardHeader>
                            <CListGroup flush>
                                { options && options.map( option => (
                                    <CListGroupItem>{option.title}</CListGroupItem>
                                ))}
                            </CListGroup>
                        </CCard>
                    )) }
